...
- splash 화면의 경우 사이트 처음 들어갔을때만 체크를 위해 세션스토리지에 값 저장
...
- 텍스트 + 이미지 업로드 가능
- 다중 이미지 미리보기/삭제 기능
- 서버 API 연동 → 게시글 등록 성공 시 피드 반영
- 기존 게시물 데이터를 불러와 수정 가능
- 제목, 내용, 이미지 교체 및 삭제 기능
- 서버 API 연동 완료
...
- 사용자별 참여 중인 채팅방 목록 표시
- 마지막 메시지, 안 읽은 메시지 여부, 상대방 프로필 이미지/닉네임 표시
- WebSocket 기반으로 최신 메시지 실시간 반영
- 사용자별 참여 중인 채팅방 목록 표시
- 마지막 메시지, 안 읽은 메시지 여부, 상대방 프로필 이미지/닉네임 표시
- WebSocket 기반으로 최신 메시지 실시간 반영
