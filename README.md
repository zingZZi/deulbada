...
- ✅ 인증 필요: Authorization 헤더에 Bearer Token 포함 필요
- ❌ 인증 불필요: 공개 API로 누구나 접근 가능
...
- **성능 최적화**: 이미지 레이지 로딩 및 메모리 효율적 데이터 관리
...
- **활동 통계**: 게시물 수, 팔로워/팔로잉 수 등 활동 지표 표시
- **생산자 인증**: 생산자 계정 전용 배지 및 상품 진열 공간
- **프라이버시 설정**: 공개/비공개 설정에 따른 접근 권한 관리
...
- **상품 정보 관리**: 가격, 수량, 설명, 카테고리 등 상세 정보 입력
- **재고 관리**: 판매 수량 및 재고 현황 실시간 업데이트
- **상품 상태 관리**: 판매중/품절/판매중지 등 상태별 관리
...
- **읽지 않은 메시지**: 안읽은 메시지 개수 표시 및 알림 배지
...
- **메시지 상태**: 전송/읽음 상태 실시간 표시
- **대화 히스토리**: 이전 대화 내용 자동 저장 및 불러오기
- **사용자 친화적 UI**: 직관적인 말풍선 디자인 및 시간 표시
- **농수산물 거래**: 상품 정보 공유 및 거래 협상 지원
- **안전한 통신**: 암호화된 메시지 전송으로 개인정보 보호
